...
const Masterpayout = () => {
  ...
  const [masterPayouts, setMasterPayouts] = useState([]);
  ...
  const loadMasterPayouts = async () => {
    try {
      if (!affiliate?.id) return;

      const token = localStorage.getItem('affiliatetoken');
      const response = await axios.get(
        `${base_url}/api/affiliate/master-payout/${affiliate.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
      ...
      if (response.data.success) {
        ...
        const processedPayouts = payoutsData.map((payout, index) => {
          let requestedAt;
          let processedAt;
          let completedAt;

          try {
            requestedAt = payout.requestedAt?.$date ? new Date(payout.requestedAt.$date) : new Date(payout.requestedAt);
            processedAt = payout.processedAt?.$date ? new Date(payout.processedAt.$date) : (payout.processedAt ? new Date(payout.processedAt) : null);
            completedAt = payout.completedAt?.$date ? new Date(payout.completedAt.$date) : (payout.completedAt ? new Date(payout.completedAt) : null);
          } catch (error) {
            console.error('Error parsing payout dates:', error);
          ...
          return {
            id: payout._id?.$oid || payout._id || `payout-${index}`,
            payoutId: payout.payoutId || `PO${Date.now()}`,
            amount: payout.amount || 0,
            netAmount: payout.netAmount || payout.amount || 0,
            ...
            includedEarnings: payout.includedEarnings || [],
            fees: payout.fees || {},
            notes: payout.notes || '',
            failureReason: payout.failureReason || '',
            failureDetails: payout.failureDetails || ''
          };
        });

        setMasterPayouts(processedPayouts);
      }
    } catch (error) {
      console.error('Error loading master payouts:', error);
  ...
  const handlePayoutStatusUpdate = async () => {
    ...
    setPayoutActionLoading('status-update');
    try {
      const token = localStorage.getItem('affiliatetoken');

      const response = await axios.put(
        `${base_url}/api/affiliate/admin/payout/${selectedPayout.id}/status`,
        {
          status: payoutStatusData.status,
          notes: payoutStatusData.notes,
          processedBy: affiliate.id
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.data.success) {
        await loadMasterPayouts();
        setShowPayoutModal(false);
        setSelectedPayout(null);
        setPayoutStatusData({ status: '', notes: '' });
        toast.success('Payout status updated successfully');
      } else {
        toast.error(response.data.message || 'Failed to update payout status');
      }
    } catch (error) {
  ...
  const openPayoutStatusModal = (payout) => {
          ...
          <div className="p-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">Master Affiliate Payouts</h2>
                  <p className="text-gray-600">Manage and track payout requests from master affiliates</p>
                </div>
                <button
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {masterPayouts.map((payout) => (
                      <tr key={payout.id} className="hover:bg-blue-50 transition-colors duration-150">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center space-x-2">
                            <button
                              onClick={() => openPayoutStatusModal(payout)}
                              ...
                              disabled={payoutActionLoading === payout.id}
                            >
                ...
                {masterPayouts.length === 0 && (
                  <div className="p-12 text-center">
                    <div className="p-8">
                      <h3 className="text-md font-medium text-gray-900 mb-2">No payout requests found</h3>
                      <p className="text-gray-500 mb-4 text-sm">
                        No payout requests from master affiliates at the moment
                      </p>
                    </div>
                  </div>
                )}
          ...
          <div className="bg-white font-poppins rounded-2xl max-w-md w-full">
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900">Update Payout Status</h3>
                    <p className="text-gray-600">Payout ID: {selectedPayout.payoutId}</p>
                  </div>
                </div>
                <button
            ...
            <div className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={payoutStatusData.status}
                    onChange={(e) => setPayoutStatusData({ ...payoutStatusData, status: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                ...
                <div>
                  ...
                  <textarea
                    value={payoutStatusData.notes}
                    onChange={(e) => setPayoutStatusData({ ...payoutStatusData, notes: e.target.value })}
                    rows="3"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                {payoutStatusData.status === 'completed' && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                    <div className="flex items-center">
                      <FaCheckCircle className="w-4 h-4 text-green-600 mr-2" />
                      <span className="text-sm text-green-800 font-medium">
                        When status is set to completed, the affiliate's balance will be updated automatically.
                      </span>
                    </div>
                  </div>
                )}
            ...
            <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
              <div className="flex justify-end space-x-3">
                ...
                <button
                  onClick={handlePayoutStatusUpdate}
                  disabled={payoutActionLoading === 'status-update'}
                  className="px-6 py-2 bg-blue-600 text-white rounded-[5px] cursor-pointer hover:bg-blue-700 transition-colors duration-200 font-medium flex items-center disabled:opacity-50"
                >
                  {payoutActionLoading === 'status-update' ? 'Updating...' : 'Update Status'}
                </button>
              </div>
            </div>
          </div>

...
const SportsGames = () => {
  ...
  const fetchGames = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${base_url}/api/admin/sports-games`);
      setGames(response.data);
    } catch (error) {
      console.error('Error fetching sports games:', error);
      toast.error('Failed to fetch sports games');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchSingleGame = async (id) => {
    try {
      const response = await axios.get(`${base_url}/api/admin/sports-games/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(`${base_url}/api/admin/sports-games/${editingId}`, formDataObj, {
          headers: {
        ...
        await axios.post(`${base_url}/api/admin/sports-games`, formDataObj, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      await axios.put(`${base_url}/api/admin/sports-games/${id}/status`, {
    ...
    try {
      ...
      if (freshGameData) {
        setFormData({
          ...
          category: freshGameData.category || 'sports',
          categoryname: freshGameData.categoryname || 'Sports',
  ...
  const deleteGame = async () => {
    ...
    try {
      await axios.delete(`${base_url}/api/admin/sports-games/${gameToDelete._id}`);
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
      ...
      await axios.put(`${base_url}/api/admin/sports-games/reorder`, {
        games: reorderedData
      });
        ...
        >
          <div className="w-full mx-auto">
            <div className="rounded-lg mb-8 flex flex-col md:flex-row justify-between items-center">
              <div>
                <h1 className="text-2xl font-semibold text-white tracking-tighter uppercase">Sports Games</h1>
                <p className="text-xs font-bold text-gray-500 mt-1">Manage sports games for the menu section</p>
              </div>
            ...
            <div className="bg-[#161B22] border border-gray-800 rounded-lg p-6 mb-8 shadow-2xl">
              <h3 className="text-xs font-black uppercase tracking-widest text-gray-400 flex items-center gap-2 mb-6">
                ...
                {isEditing ? 'Edit Sports Game' : 'Add New Sports Game'}
              </h3>
              ...
              <form onSubmit={handleSubmit}>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                    ...
                    <div className="flex-1">
                      <div className="mb-4">
                        ...
                        <label
                          htmlFor="imageInput"
                          className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 cursor-pointer transition-colors text-sm"
                        >
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 bg-[#0F111A] border border-gray-700 rounded-[3px] focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 text-gray-200"
                ...
                <div className="mb-6">
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    All games are automatically assigned to the Sports category
                  </p>
                </div>
                ...
                <div className="flex justify-end mt-8 space-x-4">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-6 py-2 bg-green-600 text-white font-medium rounded-md hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                  >
                    {loading ? (
            ...
            <div className="">
              <h3 className="text-xs font-black uppercase tracking-widest text-gray-400 flex items-center gap-2 mb-4">
                <div className="w-1 h-4 bg-green-500"></div>
                Sports Games List ({games.length})
              </h3>
              
              {loading && games.length === 0 ? (
                <div className="bg-[#161B22] rounded-lg p-12 border border-gray-800 flex items-center justify-center">
                  <div className="text-center">
                    <svg className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-green-500 mx-auto mb-4" viewBox="0 0 24 24"></svg>
                ...
                <div className="bg-[#161B22] p-12 rounded-lg text-center border border-gray-800">
                  ...
                  <p className="text-gray-500">No sports games found. Add your first game above.</p>
                </div>
              ) : (
                        ...
                        {(provided) => (
                          ...
                          >
                            <thead className="bg-[#1C2128]">
                              <tr>
                                <th className="px-4 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider w-12">
                                  #
                                </th>
                                <th className="px-6 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider w-12">
                                  <FaGripVertical className="text-gray-500" />
                                </th>
                                <th className="px-6 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider">Image</th>
                                <th className="px-6 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider">UUID</th>
                                <th className="px-6 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider">Game Name</th>
                                <th className="px-6 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider">Provider</th>
                                <th className="px-6 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider">Status</th>
                                <th className="px-6 py-4 text-left text-xs md:text-sm font-semibold text-green-400 uppercase tracking-wider">Actions</th>
                              </tr>
                            </thead>
                                  ...
                                  {(provided, snapshot) => (
                                    <tr 
                                      ...
                                      className={`hover:bg-[#1F2937] transition-colors duration-150 ${
                                        snapshot.isDragging ? 'bg-[#1F2937] shadow-2xl border-2 border-green-500' : ''
                                      }`}
                                    >
